...
7. 生成项目代码部分必须以 15 分钟内完成为目标进行时间盒控制，并严格执行“15 分钟代码生成优化要求”。
8. 生成项目时，必须调用 `design-taste-frontend` 和 `ui-ux-pro-max` 两个 skill 来提升界面设计质量。
9. 生成项目时，必须严格执行下面“端口自动适配与一次启动成功要求”。
10. 最终回复必须明确指明如何展示项目，包括启动文件名称和网址说明；启动命令和网址说明必须放在整个回复最后。
...
- 优先使用 Vite 前端项目和已有依赖，不引入大型 UI 框架或复杂状态管理，除非项目已存在并正在使用。
- 页面数量要少而完整，优先 1 个主应用页加必要组件；不要为了“看起来完整”创建大量空页面。
